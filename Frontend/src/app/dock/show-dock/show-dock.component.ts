...
@Component({
  selector: 'app-show-dock',
  templateUrl: './show-dock.component.html',
  styleUrls: ['./show-dock.component.scss']
})
export class ShowDockComponent implements OnInit {
  ...
  editClick(item){
    console.log(item);
    this.base = item;
    this.ModalTitle = 'Edit DockInfo';
    this.ActivateAddEditBaseComp = true;
  }

  deleteClick(item){
    if (confirm('Вы действительно хотите удалить эти данные?')){
      this.service.deleteDockInfo(item[0]).subscribe(data => {
        alert(data.toString());
        this.refreshBaseList();
      });
    }
  ...
  renderClick(){
    if (confirm('Вы действительно хотите опубликовать эти данные?')){
      this.service.publishDockInfo().subscribe(data => {
        alert(data.toString());
        this.refreshBaseList();
      });
    }
  ...
  refreshBaseList(){
    this.service.getDockInfoList().subscribe(data => {
      const arr: any[] = [];

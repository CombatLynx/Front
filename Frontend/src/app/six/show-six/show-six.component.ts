...
@Component({
  selector: 'app-show-six',
  templateUrl: './show-six.component.html',
  styleUrls: ['./show-six.component.scss']
})
export class ShowSixComponent implements OnInit {
  ...
  deleteClick(item){
    if (confirm('Вы действительно хотите удалить эти данные?')){
      this.service.deleteSixsInfo(item[0]).subscribe(data => {
        alert(data.toString());
        this.refreshBaseList();
      });
    }
  ...
  renderClick(){
    if (confirm('Вы действительно хотите опубликовать эти данные?')){
      this.service.publishSixsInfo().subscribe(data => {
        alert(data.toString());
        this.refreshBaseList();
      });
    }
  ...
  refreshBaseList(){
    this.service.getSixsInfoList().subscribe(data => {
      const arr: any[] = [];

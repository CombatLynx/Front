...
@Component({
  ...
  styleUrls: ['./show-facilities.component.scss']
})
export class ShowFacilitiesComponent implements OnInit {
  ...
  deleteClick(item){
    if (confirm('Вы действительно хотите удалить эти данные?')){
      this.service.deleteFacilitInfo(item[0]).subscribe(data => {
        alert(data.toString());
        this.refreshBaseList();
      });
    }
  ...
  renderClick(){
    if (confirm('Вы действительно хотите опубликовать эти данные?')){
      this.service.publishFacilitInfo().subscribe(data => {
        alert(data.toString());
        this.refreshBaseList();
      });
    }
  ...
  refreshBaseList(){
    this.service.getFacilitInfoList().subscribe(data => {
      const arr: any[] = [];

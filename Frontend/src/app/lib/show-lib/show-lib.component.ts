...
export class ShowLibComponent implements OnInit {
  ...
  addClick(){
    this.base = {};
    this.ModalTitle = 'Add SpecLibsInfo';
  ...
  editClick(item){
    console.log(item);
    this.base = item;
    this.ModalTitle = 'Edit SpecLibsInfo';
    this.ActivateAddEditBaseComp = true;
  }

  deleteClick(item){
    if (confirm('Вы действительно хотите удалить эти данные?')){
      this.service.deleteSpecLibsInfo(item[0]).subscribe(data => {
        alert(data.toString());
        this.refreshBaseList();
      });
    }
  ...
  renderClick(){
    if (confirm('Вы действительно хотите опубликовать эти данные?')){
      this.service.publishSpecLibsInfo().subscribe(data => {
        alert(data.toString());
        this.refreshBaseList();
      });
    }
  ...
  refreshBaseList(){
    this.service.getSpecLibsInfoList().subscribe(data => {
      const arr: any[] = [];

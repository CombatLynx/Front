...
@Component({
  ...
  styleUrls: ['./show-teach.component.scss']
})
export class ShowTeachComponent implements OnInit {
  ...
  addClick(){
    this.base = {};
    this.ModalTitle = 'Add TeacherInfo';
  ...
  editClick(item){
    console.log(item);
    this.base = item;
    this.ModalTitle = 'Edit TeacherInfo';
    this.ActivateAddEditBaseComp = true;
  }

  deleteClick(item){
    if (confirm('Вы действительно хотите удалить эти данные?')){
      this.service.deleteTeacherInfo(item[0]).subscribe(data => {
        alert(data.toString());
        this.refreshBaseList();
      });
    }
  ...
  renderClick(){
    if (confirm('Вы действительно хотите опубликовать эти данные?')){
      this.service.publishTeacherInfo().subscribe(data => {
        alert(data.toString());
        this.refreshBaseList();
      });
    }
  ...
  refreshBaseList(){
    this.service.getTeacherInfoList().subscribe(data => {
      const arr: any[] = [];

...
@Component({
  ...
  styleUrls: ['./add-edit-teach.component.scss']
})
export class AddEditTeachComponent implements OnInit {
  ...
  addTeacher(){
    ...
    console.log(json);
    this.service.addTeacherInfoList(json.id, json).subscribe(res => {
      alert(res.toString());
    });
  }
  ...
  updateTeacher(){
    ...
    this.service.updateTeacherInfo(json.id, json).subscribe(res => {
      alert(res.toString());
      console.log(this.Files);
  ...
  refreshBaseList(){
    this.service.getTeacherInfoFormat().subscribe(data => {

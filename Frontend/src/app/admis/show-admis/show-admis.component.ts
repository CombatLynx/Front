...
@Component({
  selector: 'app-show-admis',
  templateUrl: './show-admis.component.html',
  styleUrls: ['./show-admis.component.scss']
})
export class ShowAdmisComponent implements OnInit {
  ...
  deleteClick(item){
    if (confirm('Вы действительно хотите удалить эти данные?')){
      this.service.deleteAdmisInfo(item[0]).subscribe(data => {
        alert(data.toString());
        this.refreshBaseList();
      });
    }
  ...
  renderClick(){
    if (confirm('Вы действительно хотите опубликовать эти данные?')){
      this.service.publishAdmisInfo().subscribe(data => {
        alert(data.toString());
        this.refreshBaseList();
      });
    }
  ...
  refreshBaseList(){
    this.service.getAdmisInfoList().subscribe(data => {
      const arr: any[] = [];

...
@Component({
  selector: 'app-show-doco',
  templateUrl: './show-doco.component.html',
  styleUrls: ['./show-doco.component.scss']
})
export class ShowDocoComponent implements OnInit {
  ...
  editClick(item){
    console.log(item);
    this.base = item;
    this.ModalTitle = 'Edit DocoInfo';
    this.ActivateAddEditBaseComp = true;
  }

  deleteClick(item){
    if (confirm('Вы действительно хотите удалить эти данные?')){
      this.service.deleteDocoInfo(item[0]).subscribe(data => {
        alert(data.toString());
        this.refreshBaseList();
      });
    }
  ...
  renderClick(){
    if (confirm('Вы действительно хотите опубликовать эти данные?')){
      this.service.publishDocoInfo().subscribe(data => {
        alert(data.toString());
        this.refreshBaseList();
      });
    }
  ...
  refreshBaseList(){
    this.service.getDocoInfoList().subscribe(data => {
      const arr: any[] = [];

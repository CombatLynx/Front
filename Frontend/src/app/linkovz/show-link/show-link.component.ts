...
export class ShowLinkComponent implements OnInit {
  ...
  addClick(){
    this.base = {};
    this.ModalTitle = 'Add LinkOvzsInfo';
  ...
  editClick(item){
    console.log(item);
    this.base = item;
    this.ModalTitle = 'Edit LinkOvzsInfo';
    this.ActivateAddEditBaseComp = true;
  }

  deleteClick(item){
    if (confirm('Вы действительно хотите удалить эти данные?')){
      this.service.deleteLinkOvzsInfo(item[0]).subscribe(data => {
        alert(data.toString());
        this.refreshBaseList();
      });
    }
  ...
  renderClick(){
    if (confirm('Вы действительно хотите опубликовать эти данные?')){
      this.service.publishLinkOvzsInfo().subscribe(data => {
        alert(data.toString());
        this.refreshBaseList();
      });
    }
  ...
  refreshBaseList(){
    this.service.getLinkOvzsInfoList().subscribe(data => {
      const arr: any[] = [];

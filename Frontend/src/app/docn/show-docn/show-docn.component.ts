...
@Component({
  selector: 'app-show-docn',
  templateUrl: './show-docn.component.html',
  styleUrls: ['./show-docn.component.scss']
})
export class ShowDocnComponent implements OnInit {
  ...
  editClick(item){
    console.log(item);
    this.base = item;
    this.ModalTitle = 'Edit DocnInfo';
    this.ActivateAddEditBaseComp = true;
  }

  deleteClick(item){
    if (confirm('Вы действительно хотите удалить эти данные?')){
      this.service.deleteDocnInfo(item[0]).subscribe(data => {
        alert(data.toString());
        this.refreshBaseList();
      });
    }
  ...
  renderClick(){
    if (confirm('Вы действительно хотите опубликовать эти данные?')){
      this.service.publishDocnInfo().subscribe(data => {
        alert(data.toString());
        this.refreshBaseList();
      });
    }
  ...
  refreshBaseList(){
    this.service.getDocnInfoList().subscribe(data => {
      const arr: any[] = [];

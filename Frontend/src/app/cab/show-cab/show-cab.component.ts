...
export class ShowCabComponent implements OnInit {
  ...
  deleteClick(item){
    if (confirm('Вы действительно хотите удалить эти данные?')){
      this.service.deleteSpecCabsInfo(item[0]).subscribe(data => {
        alert(data.toString());
        this.refreshBaseList();
      });
    }
  ...
  renderClick(){
    if (confirm('Вы действительно хотите опубликовать эти данные?')){
      this.service.publishSpecCabsInfo().subscribe(data => {
        alert(data.toString());
        this.refreshBaseList();
      });
    }
  ...
  refreshBaseList(){
    this.service.getSpecCabsInfoList().subscribe(data => {
      const arr: any[] = [];

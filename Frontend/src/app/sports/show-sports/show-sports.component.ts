...
@Component({
  ...
  styleUrls: ['./show-sports.component.scss']
})
export class ShowSportsComponent implements OnInit {
  ...
  addClick(){
    this.base = {};
    this.ModalTitle = 'Add SportsInfo';
  ...
  editClick(item){
    console.log(item);
    this.base = item;
    this.ModalTitle = 'Edit SportsInfo';
    this.ActivateAddEditBaseComp = true;
  }

  deleteClick(item){
    if (confirm('Вы действительно хотите удалить эти данные?')){
      this.service.deleteSportsInfo(item[0]).subscribe(data => {
        alert(data.toString());
        this.refreshBaseList();
      });
    }
  ...
  renderClick(){
    if (confirm('Вы действительно хотите опубликовать эти данные?')){
      this.service.publishSportsInfo().subscribe(data => {
        alert(data.toString());
        this.refreshBaseList();
      });
    }
  ...
  refreshBaseList(){
    this.service.getSportsInfoList().subscribe(data => {
      const arr: any[] = [];

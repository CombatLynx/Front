...
export class ShowMealComponent implements OnInit {
  ...
  addClick(){
    this.base = {};
    this.ModalTitle = 'Add SpecMealsInfo';
  ...
  editClick(item){
    console.log(item);
    this.base = item;
    this.ModalTitle = 'Edit SpecMealsInfo';
    this.ActivateAddEditBaseComp = true;
  }

  deleteClick(item){
    if (confirm('Вы действительно хотите удалить эти данные?')){
      this.service.deleteSpecMealsInfo(item[0]).subscribe(data => {
        alert(data.toString());
        this.refreshBaseList();
      });
    }
  ...
  renderClick(){
    if (confirm('Вы действительно хотите опубликовать эти данные?')){
      this.service.publishSpecMealsInfo().subscribe(data => {
        alert(data.toString());
        this.refreshBaseList();
      });
    }
  ...
  refreshBaseList(){
    this.service.getSpecMealsInfoList().subscribe(data => {
      const arr: any[] = [];

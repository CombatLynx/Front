...
@Component({
  selector: 'app-show-doch',
  templateUrl: './show-doch.component.html',
  styleUrls: ['./show-doch.component.scss']
})
export class ShowDochComponent implements OnInit {
  ...
  editClick(item){
    console.log(item);
    this.base = item;
    this.ModalTitle = 'Edit DochInfo';
    this.ActivateAddEditBaseComp = true;
  }

  deleteClick(item){
    if (confirm('Вы действительно хотите удалить эти данные?')){
      this.service.deleteDochInfo(item[0]).subscribe(data => {
        alert(data.toString());
        this.refreshBaseList();
      });
    }
  ...
  renderClick(){
    if (confirm('Вы действительно хотите опубликовать эти данные?')){
      this.service.publishDochInfo().subscribe(data => {
        alert(data.toString());
        this.refreshBaseList();
      });
    }
  ...
  refreshBaseList(){
    this.service.getDochInfoList().subscribe(data => {
      const arr: any[] = [];
